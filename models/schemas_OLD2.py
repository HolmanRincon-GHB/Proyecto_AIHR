...
class RRHHRequest(BaseModel):
    ...
    class Config:
        json_schema_extra = {
            "example": {
                "pregunta":   "¿Qué área tiene más riesgo de renuncia?",
                "session_id": "rrhh_001"
            }
        }
...
class RRHHResponse(BaseModel):
    """Respuesta del POST /rrhh"""
    respuesta:  str      = Field(..., description="Respuesta del equipo multiagente de RRHH")
    pregunta:   str      = Field(..., description="Pregunta original")
    session_id: str      = Field(..., description="ID de sesión usado")
    timestamp:  datetime = Field(default_factory=datetime.now)
    fuentes:    List[str] = Field(default_factory=list, description="Documentos internos de RRHH citados")

    class Config:
        json_schema_extra = {
            "example": {
                "respuesta":  "El área con mayor riesgo de renuncia es Logística (34% de riesgo promedio)...",
                "pregunta":   "¿Qué área tiene más riesgo de renuncia?",
                "session_id": "rrhh_001",
                "timestamp":  "2025-01-15T10:30:00",
                "fuentes":    ["Análisis de Riesgo de Rotación por Área"]
            }
        }
